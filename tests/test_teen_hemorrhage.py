...
async def test_teen_hemorrhage():
    ...
    try:
        ...
        risk_keywords = ['high risk', 'high-risk', 'urgent', 'emergency', 'immediately', 'right away']
        risk_detected = any(keyword in full_response.lower() for keyword in risk_keywords)
        ...
        nurse_keywords = ['nurse', 'doctor', 'healthcare provider', 'medical professional', 'specialist']
        nurse_mentioned = any(keyword in full_response.lower() for keyword in nurse_keywords)
        ...
        facility_keywords = ['hospital', 'clinic', 'health facility', 'emergency room', 'medical center']
        facility_mentioned = any(keyword in full_response.lower() for keyword in facility_keywords)
        ...
        empathy_detected = any(keyword in full_response.lower() for keyword in empathy_keywords)
        ...
        teen_aware = ('17' in full_response or 'young' in full_response.lower() or 
                      'parent' in full_response.lower() or 'guardian' in full_response.lower())
        
        if teen_aware:
            criteria_met.append("✅ Age-appropriate communication")
        else:
            criteria_failed.append("⚠️  Age-specific considerations not explicitly mentioned")
