...
async def test_missing_lmp():
    ...
    try:
        ...
        lmp_questions = ['last menstrual period', 'lmp', 'last period', 'when was your last period', 
                        'first day of your last period', 'date of your last period']
        ...
        supportive_words = ['congratulations', 'happy', 'help', 'support', 'guide', 'assist']
        supportive_tone = any(word in full_response_1.lower() for word in supportive_words)
        ...
        format_guidance = ['format' in full_response_1.lower() or 
                          'yyyy-mm-dd' in full_response_1.lower() or
                          'ultrasound' in full_response_1.lower() or
                          'scan' in full_response_1.lower()]
        ...
        edd_mentioned = any(phrase in combined_response.lower() for phrase in 
                          ['due date', 'estimated delivery', 'edd', 'expected delivery', 
                           'baby will be born', 'delivery date'])
        ...
        next_steps = any(phrase in combined_response.lower() for phrase in 
                        ['anc', 'antenatal', 'appointment', 'visit', 'checkup', 
                         'schedule', 'care plan', 'next step'])
