...
async def test_low_risk():
    ...
    try:
        ...
        reassuring_words = ['congratulations', 'normal', 'healthy', 'wonderful', 'great', 
                           'routine', 'regular', 'well', 'good']
        ...
        if reassurance_detected and not alarm_detected:
            ...
            criteria_failed.append("❌ Inappropriate alarm language used for low-risk case")
        else:
            criteria_failed.append("⚠️  No reassurance detected")
        
        # 2. Check if EDD was calculated
        edd_keywords = ['due date', 'estimated delivery', 'edd', 'expected delivery', 
                       'december', 'dec']  # March 1 + 280 days ≈ Dec 6
        edd_mentioned = any(keyword in full_response.lower() for keyword in edd_keywords)
        ...
        anc_keywords = ['anc', 'antenatal', 'checkup', 'appointment', 'visit', 
                       'schedule', 'weeks', '8 weeks', '12 weeks', '20 weeks']
        anc_mentioned = any(keyword in full_response.lower() for keyword in anc_keywords)
        ...
        advice_keywords = ['nutrition', 'diet', 'exercise', 'rest', 'sleep', 'vitamins', 
                          'folic acid', 'iron', 'water', 'hydration', 'avoid', 'healthy']
        advice_given = any(keyword in full_response.lower() for keyword in advice_keywords)
        ...
        next_steps = ['clinic' in full_response.lower() or 
                     'hospital' in full_response.lower() or
                     'doctor' in full_response.lower() or
                     'midwife' in full_response.lower() or
                     'healthcare' in full_response.lower() or
                     'facility' in full_response.lower()]
        ...
        risk_keywords = ['low risk', 'low-risk', 'normal pregnancy', 'healthy pregnancy']
        risk_communicated = any(keyword in full_response.lower() for keyword in risk_keywords)
