...
async def test_invalid_date():
    ...
    try:
        ...
        user_input_1 = (
            "Hi, I'm pregnant. My last period was yesterday."
        )
        ...
        polite_words = ['could you', 'please', 'would you mind', 'help me understand', 
                       'clarify', 'provide', 'share']
        ...
        format_guidance = ['yyyy-mm-dd' in all_responses.lower() or
                          'format' in all_responses.lower() or
                          '2025-03-01' in all_responses.lower() or
                          'year-month-day' in all_responses.lower()]
        ...
        error_indicators = ['error occurred', 'crashed', 'failed to', 'unable to process']
        no_crash = not any(indicator in all_responses.lower() for indicator in error_indicators)
        ...
        clarification_words = ['clarify', 'confirm', 'verify', 'check', 'specific date', 
                              'exact date', 'actual date']
        clarification_asked = any(word in (full_response_1 + full_response_2).lower() 
        ...
        success_detected = any(indicator in full_response_3.lower() for indicator in success_indicators)
        ...
        educational_words = ['because', 'so that', 'in order to', 'helps me', 'allows me', 
                            'calculate', 'determine', 'important']
        educational_detected = any(word in all_responses.lower() for word in educational_words)
